...
body {
  ...
  background:
    radial-gradient(circle at 12% 14%, rgba(99, 102, 241, 0.12), transparent 28%),
    radial-gradient(circle at 89% 86%, rgba(139, 92, 246, 0.08), transparent 26%),
    radial-gradient(circle at 52% 32%, rgba(56, 189, 248, 0.05), transparent 30%),
    linear-gradient(135deg, #020617 0%, #09090b 100%);
...
.nav-button:hover, .stage-close:hover {
...
.workspace {
  display: grid;
  grid-template-columns: minmax(430px, 35%) minmax(0, 1fr);
  min-height: 0;
}
...
.right-pane::before {
  ...
  inset: 0;
  background:
    radial-gradient(circle at 70% 30%, rgba(99, 102, 241, 0.05), transparent 50%),
    linear-gradient(135deg, rgba(2, 6, 23, 0.8), rgba(9, 9, 11, 0.8));
  pointer-events: none;
  z-index: -1;
}
...
.tv-screen.busy::after {
  ...
  background: linear-gradient(180deg, 
      transparent 0%, 
      rgba(99, 102, 241, 0.05) 10%, 
      rgba(99, 102, 241, 0.2) 50%, 
      rgba(99, 102, 241, 0.05) 90%, 
      transparent 100%);
  height: 20%;
  width: 100%;
...
.cursor-dot {
  ...
  box-shadow: 0 0 0 4px rgba(99, 102, 241, 0.2), 0 8px 24px rgba(0, 0, 0, 0.4);
  transform: translate(-50%, -50%);
  display: none;
  transition: left 250ms cubic-bezier(0.4, 0, 0.2, 1), top 250ms cubic-bezier(0.4, 0, 0.2, 1), opacity 200ms ease;
  z-index: 10;
}
.cursor-dot.pending {
...
@keyframes pulse {
  0%, 100% { box-shadow: 0 0 0 4px rgba(99, 102, 241, 0.2), 0 8px 24px rgba(0,0,0,0.4); }
  50% { box-shadow: 0 0 0 10px rgba(99, 102, 241, 0.4), 0 8px 24px rgba(0,0,0,0.4); }
}
.cursor-bubble {
  position: absolute;
...
.timeline-handle {
  ...
  border-radius: 999px;
  background: #fff;
  box-shadow: 0 0 0 2px rgba(139, 92, 246, 0.3), 0 4px 12px rgba(0, 0, 0, 0.5);
  transform: translate(-50%, -50%);
  pointer-events: none;
  z-index: 2;
...
.status-pill.running,
.thread-chip.running,
.thread-chip.blocked_waiting_user { 
  color: #fbbf24; 
...
.token-stat-value.green { color: #34d399; }
.token-stat-value.yellow { color: #fbbf24; }
.token-stat-value.red { color: #f87171; }
.token-track-outer {
  height: 6px;
  border-radius: 999px;
  background: rgba(255,255,255,0.06);
...
.chat-label.tool-label { color: #22d3ee; }
.chat-label.done-label { color: #34d399; }
.chat-label.error-label { color: #f87171; }
...
.terminal-line {
  ...
  white-space: pre-wrap;
  word-break: break-all;
  animation: termLine 0.15s ease;
}
@keyframes termLine {
  from { opacity: 0; transform: translateX(-4px); }
  to { opacity: 1; transform: translateX(0); }
}
.terminal-line.thought { color: #a78bfa; }
.terminal-line.tool { color: #22d3ee; }
.terminal-line.result-ok { color: #34d399; }
.terminal-line.result-err { color: #f87171; }
.terminal-line.done { color: #34d399; font-weight: 700; }
.terminal-line.meta { color: rgba(148,163,184,0.5); }
.terminal-prompt { color: #4ade80; margin-right: 6px; }
...
function countTokens() {
  ...
  for (const ev of state.events) {
    if (ev.usage) {
      total += (ev.usage.prompt_tokens || 0) + (ev.usage.completion_tokens || 0);
    }
  }
  return total;
}
...
function countToolCalls() {
  return state.events.filter(e => e.type === 'tool_call').length;
}

function countThoughts() {
  return state.events.filter(e => e.type === 'thought').length;
}

function renderTokenBar() {
  const el = document.getElementById('token-bar-wrap');
  if (!el) return;
  const tokens = countTokens();
  const steps = state.steps.length;
  const tools = countToolCalls();
  const maxTokens = 128000;
  const pct = Math.min(100, (tokens / maxTokens) * 100).toFixed(1);
  const color = tokens > 100000 ? 'red' : tokens > 60000 ? 'yellow' : 'green';
  el.innerHTML = `
    <div class="token-bar">
      <div class="token-stats">
        <div class="token-stat">
          <div class="token-stat-value ${color}">${fmtNum(tokens)}</div>
          <div class="token-stat-label">Token</div>
        </div>
        <div class="token-stat">
          <div class="token-stat-value">${steps}</div>
          <div class="token-stat-label">Adım</div>
        </div>
        <div class="token-stat">
          <div class="token-stat-value">${tools}</div>
          <div class="token-stat-label">Araç</div>
        </div>
      </div>
      <div class="token-track-outer">
        <div class="token-track-fill" style="width:${pct}%"></div>
      </div>
    </div>
  `;
}
...
function renderModelPicker() {
  const el = document.getElementById('model-picker-wrap');
  if (!el) return;
  const providers = [
    { id: 'openai', label: 'OpenAI', sub: 'GPT-4o' },
    { id: 'anthropic', label: 'Claude', sub: 'Sonnet' },
    { id: 'gemini', label: 'Gemini', sub: 'Flash' },
    { id: 'ollama', label: 'Ollama', sub: 'Local' },
  ];
  el.innerHTML = `<div class="model-switcher">${providers.map(p => `
    <button
      type="button"
      class="model-tab ${state.provider === p.id ? 'active' : ''}"
      data-model-pick="${p.id}">
      ${p.label}<span class="model-sub">${p.sub}</span>
    </button>`).join('')}</div>`;
}

function renderChatTimeline() {
  const el = document.getElementById('chat-timeline-wrap');
  if (!el) return;
  const visible = state.events.filter(e =>
    ['thought','tool_call','tool_result','done','error'].includes(e.type)
  ).slice(-20);
  if (!visible.length) {
    el.innerHTML = '<div class="empty-state">Henüz düşünce yok. Run başlat.</div>';
    return;
  }
  el.innerHTML = `<div class="chat-timeline">${visible.map((e, i) => {
    const ts = e.timestamp ? new Date(e.timestamp).toLocaleTimeString('tr',{hour:'2-digit',minute:'2-digit',second:'2-digit'}) : '';
    if (e.type === 'thought') {
      const text = String(e.display_summary || e.content || '').trim();
      return `<div class="chat-bubble">
        <div class="chat-avatar">🧠</div>
        <div class="chat-text-wrap">
          <div class="chat-label">${escapeHtml(e.display_label || 'Düşünce')}</div>
          <div class="chat-text">${escapeHtml(shortText(text, 280))}</div>
          ${ts ? `<div class="chat-timestamp">${ts}</div>` : ''}
        </div>
      </div>`;
    }
    if (e.type === 'tool_call') {
      return `<div class="chat-bubble">
        <div class="chat-avatar chat-tool-avatar">⚙️</div>
        <div class="chat-text-wrap">
          <div class="chat-label tool-label">${escapeHtml(e.display_label || e.tool || 'Araç')}</div>
          <div class="chat-text tool-text">${escapeHtml(shortText(e.display_summary || '', 180))}</div>
          ${ts ? `<div class="chat-timestamp">${ts}</div>` : ''}
        </div>
      </div>`;
    }
    if (e.type === 'done') {
      return `<div class="chat-bubble">
        <div class="chat-avatar" style="background:linear-gradient(135deg,#10b981,#047857);">✅</div>
        <div class="chat-text-wrap">
          <div class="chat-label done-label">Tamamlandı</div>
          <div class="chat-text done-text">${escapeHtml(shortText(e.content || '', 280))}</div>
          ${ts ? `<div class="chat-timestamp">${ts}</div>` : ''}
        </div>
      </div>`;
    }
    if (e.type === 'error') {
      return `<div class="chat-bubble">
        <div class="chat-avatar" style="background:linear-gradient(135deg,#ef4444,#b91c1c);">❌</div>
        <div class="chat-text-wrap">
          <div class="chat-label error-label">Hata</div>
          <div class="chat-text error-text">${escapeHtml(shortText(e.content || '', 200))}</div>
          ${ts ? `<div class="chat-timestamp">${ts}</div>` : ''}
        </div>
      </div>`;
    }
  ...
  const wrap = el.querySelector('.chat-timeline');
  if (wrap) wrap.scrollTop = wrap.scrollHeight;
}

function renderTerminalLog() {
  const el = document.getElementById('terminal-log-wrap');
  if (!el) return;
  const lines = state.events.filter(e =>
    ['thought','tool_call','tool_result','done','error','phase'].includes(e.type)
  ).slice(-60);
  ...
  el.innerHTML = `
    <div class="terminal-box">
      <div class="terminal-header">
        ...
        <div class="terminal-dot" style="background:#fbbf24"></div>
        <div class="terminal-dot" style="background:#34d399"></div>
        <span class="terminal-title">agentra — canlı log</span>
      </div>
      <div class="terminal-scroll" id="terminal-scroll-inner">
        ${lines.length === 0 ? '<span class="terminal-line meta">// Run başlatmayı bekliyorum...</span>' : ''}
        ${lines.map((e, i) => {
          const last = i === lines.length - 1;
          const cursor = last && isRunning ? ' cursor' : '';
          if (e.type === 'thought') {
            const txt = shortText(e.display_summary || e.content || '', 140);
            return `<div class="terminal-line thought${cursor}"><span class="terminal-prompt">$</span><span style="opacity:.5">thought</span>  ${escapeHtml(txt)}</div>`;
          }
          if (e.type === 'tool_call') {
            return `<div class="terminal-line tool${cursor}"><span class="terminal-prompt">→</span><span style="opacity:.5">${escapeHtml(e.tool||'tool')}</span>  ${escapeHtml(shortText(e.display_summary||'',100))}</div>`;
          }
          if (e.type === 'tool_result') {
            const ok = e.success !== false;
            return `<div class="terminal-line ${ok?'result-ok':'result-err'}${cursor}"><span class="terminal-prompt">${ok?'✓':'✗'}</span>  ${escapeHtml(shortText(e.display_summary||'',120))}</div>`;
          }
          if (e.type === 'done') {
            return `<div class="terminal-line done${cursor}"><span class="terminal-prompt">✅</span>  ${escapeHtml(shortText(e.content||'Görev tamamlandı.',120))}</div>`;
          }
          if (e.type === 'error') {
            return `<div class="terminal-line result-err${cursor}"><span class="terminal-prompt">✗✗</span>  ${escapeHtml(shortText(e.content||'Hata.',120))}</div>`;
          }
          if (e.type === 'phase') {
            return `<div class="terminal-line meta">// ${escapeHtml(shortText(e.summary||e.content||'',80))}</div>`;
          }
          return '';
        }).join('')}
      </div>
  ...
  `;
  const inner = document.getElementById('terminal-scroll-inner');
...
function mount() {
      ...
      <section class="workspace">
        <section class="left-pane">
          <div class="left-pane-inner">
            <section class="console-section" id="model-picker-section">
              <div class="section-head">
                <div class="section-title">Model</div>
              </div>
              <div id="model-picker-wrap"></div>
            </section>

            <section class="console-section" id="token-stats-section">
              <div class="section-head">
                <div class="section-title">Token & İstatistikler</div>
              </div>
              <div id="token-bar-wrap"></div>
            </section>
            ...
            <section class="console-section">
              <div class="section-head">
                <div class="section-title">Düşünce Akışı</div>
              </div>
              <div id="chat-timeline-wrap"></div>
            </section>
  ...
  document.addEventListener('click', (e) => {
    const pick = e.target.closest('[data-model-pick]');
    if (pick) {
      state.provider = pick.getAttribute('data-model-pick');
